...
namespace UnitTestMinesweeper.Tests
{
    ...
    public class BoardTests
    {
        ...
        [TestMethod]
        public void ItCountsMinesAroundCorrectly()
        {
            // Arrange
        ...
        public void ItCountsMinesAroundCorrectly_3()
        {
            // Arrange
            int h = 4; int w = 4;
            Board board = new Board(w, h, 3);
            board.Cells = MakeGrid(w,h, new List<int[]> { new int[] { 0, 1 }, new int[] { 1, 1 }, new int[] { 1, 0 }});
            // Act
            board.CreateNumThatShowsMinesAround();
            PrintOutBoardCellsContentInsideTestForDebugging(board);
            // Assert
            /*
             [3,*,2,0],
             [*,*,2,0],
            ...
            board.Cells[1, 2].NumMines.Should().Be(2);
            board.Cells[1, 3].NumMines.Should().Be(0);

            board.Cells[2, 0].NumMines.Should().Be(2);
            board.Cells[2, 1].NumMines.Should().Be(2);
            board.Cells[2, 2].NumMines.Should().Be(1);

            board.Cells[3, 3].NumMines.Should().Be(0);
        }
